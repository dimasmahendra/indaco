		...
		<?php if(count($subscriber) > 0): ?>
			...
			<tr>
				...
				<td>
					...
					<button class="btn btn-sm btn-danger" onclick="confirm_del('<?= $value['email'] ?>','<?= $value['id'] ?>')">delete</button>
				</td>
			</tr>
			<?php endforeach ?>
		<?php else: ?>
...
<script type="text/javascript">
	...
	function delete_data(delete_id){
		$.ajax(
			{
				url : '<?= site_url('ctrsubscriber/delete')?>',
				type: 'post',
				data : {'id' : delete_id},
				...
				success: function(result,status,xhr){
					...
					if(hasil.status == '0'){
						...
						setTimeout(function(){
							window.location.replace('<?= site_url('ctrsubscriber') ?>');
						}, 1000);
					}
				},
