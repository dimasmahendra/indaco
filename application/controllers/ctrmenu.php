...
//  $this->load->view('test.php');
   ...
   function getArrayJmlGambar() {
    ...
    function setDetailFormmenu($xidmenu) {
        ...
        $xBufResult .= '<div class="garis"></div>' . form_button('btSimpan', 'Simpan', 'onclick="dosimpanmenu();"') . form_button('btNew', 'Baru', 'onclick="doClearmenu();"') . '<div class="spacer"></div><div id="tabledatamenu">' . $this->getlistmenu(0, '') . '</div><div class="spacer"></div>';
        return $xBufResult;
    }
...
//$xButtonSearch = '<img src="' . base_url() . 'resource/imgbtn/b_view.png" alt="Search Data" onclick = "dosearchmenu(0);" style="border:none;width:30px;height:30px;" />';
    ...
    function simpanmenu() {
        ...
        if (!empty($idpegawai)) {
            if ($xidmenu != '0') {
                $xStr = $this->modelmenu->setUpdatemenu($xidmenu, $xnmmenu, $xtipemenu, $xidkomponen, $xiduser, $xparentmenu, $xurlci, $xurut, $xjmlgambar,$xsettingform);
            } else {
                $xStr = $this->modelmenu->setInsertmenu($xidmenu, $xnmmenu, $xtipemenu, $xidkomponen, $xiduser, $xparentmenu, $xurlci, $xurut, $xjmlgambar,$xsettingform);
            }
        }
        echo json_encode(null);
    }
