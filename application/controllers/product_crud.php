...
class product_crud extends CI_Controller {
	...
	{	
		#get data
		$return = array();
		$return['data'] = $this->db->get('in_products')->result_array();
		$this->load->view('product_crud/index',$return); 			
	}	
	...
	public function typeOptions(){
		...
		$in_product_type = $this->db->get('in_product_type')->result_array();
	...
	public function features(){
		...
		$in_product_type = $this->db->get('in_product_feature')->result_array();
		if(count($in_product_type) > 0){
			foreach ($in_product_type as $key => $value) {
				$result[$value['id']] = $value['ind_name'];
			}	
		}
	...
	public function color_images($product_id){
		$this->db->where('product_id', $product_id);
		$in_product_color = $this->db->get('in_product_color')->result_array();		
		return $in_product_color;
	}

	public function docs_product($product_id){
		$this->db->where('product_id', $product_id);
		$in_product_file = $this->db->get('in_product_file')->result_array();		
		return $in_product_file;
	}
	...
	public function form_add(){
		...
		$this->load->view('product_crud/form_add',$return);
	}
