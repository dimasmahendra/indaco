...
//        $xBufResult .= setForm('edisaktif', 'isaktif', form_input(getArrayObj('edisaktif', '', '100'))) . '<div class="spacer"></div>';
        ...
        $xBufResult .= '<div class="garis"></div>' . form_button('btSimpan', 'Simpan', 'onclick="dosimpanusersistem();"') . form_button('btNew', 'Baru', 'onclick="doClearusersistem();"') . '<div class="spacer"></div><div id="tabledatausersistem">' . $this->getlistusersistem(0, '') . '</div><div class="spacer"></div>';
        return $xBufResult;
    }
    ...
    function getlistusersistem($xAwal, $xSearch) {
        ...
        foreach ($xQuery->result() as $row) {
            $xButtonEdit = '<img src="' . base_url() . 'resource/imgbtn/edit.png" alt="Edit Data" onclick = "doeditusersistem(\'' . $row->idx . '\');" style="border:none;width:20px"/>';
            $xButtonHapus = '<img src="' . base_url() . 'resource/imgbtn/delete_table.png" alt="Hapus Data" onclick = "dohapususersistem(\'' . $row->idx . '\',\'' . substr($row->npp, 0, 20) . '\');" style="border:none;">';
...
//$xButtonSearch = '<img src="' . base_url() . 'resource/imgbtn/b_view.png" alt="Search Data" onclick = "dosearchusersistem(0);" style="border:none;width:30px;height:30px;" />';
        $xButtonPrev = '<img src="' . base_url() . 'resource/imgbtn/b_prevpage.png" style="border:none;width:20px;" onclick = "dosearchusersistem(' . ($xAwal - $xLimit) . ');"/>';
        $xButtonNext = '<img src="' . base_url() . 'resource/imgbtn/b_nextpage.png" style="border:none;width:20px;" onclick = "dosearchusersistem(' . ($xAwal + $xLimit) . ');" />';
    ...
    function simpanusersistem() {
        ...
        if (!empty($idpegawai)) {
            if ($xidx != '0') {
                $xStr = $this->modelusersistem->setUpdateusersistem($xidx, $xnpp, $xNama, $xalamat, $xNoTelpon, $xuser,
                        $xpassword, $xstatuspeg, $xphoto, $xemail, $xym, $xisaktif, $xidusergroup,$xidkabupaten,$xidpropinsi,$ximehp);
            } else {
                $xStr = $this->modelusersistem->setInsertusersistem($xidx, $xnpp, $xNama, $xalamat, $xNoTelpon, $xuser,
                        $xpassword, $xstatuspeg, $xphoto, $xemail, $xym, $xisaktif, $xidusergroup,$xidkabupaten,$xidpropinsi,$ximehp);
            }
        }
