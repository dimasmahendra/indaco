...
class modelkabupaten extends CI_Model {
    ...
    function getArrayListkabupaten() { /* spertinya perlu lock table */
        ...
        foreach ($query->result() as $row) {
            $xBuffResul[$row->kode_kabupaten] = $row->kabupaten;
        }
        return $xBuffResul;
    }
    function getArrayListkabupatenkota() { /* spertinya perlu lock table */
        $xBuffResul = array();
        $xStr = "SELECT " .
                "kabupaten.idx" . ",kode_kabupaten" .",kode_kota" .
                ",kabupaten" .
                ",kabupaten.idprovinsi " .
                " FROM kabupaten,kota group by kabupaten.idx order by kabupaten.idx ASC ";
        $query = $this->db->query($xStr);
        $xBuffResul[0] = '--kabupaten--';
        foreach ($query->result() as $row) {
            $xBuffResul[$row->kode_kabupaten] = $row->kabupaten;
            if (!empty($row->kota)){
            $xBuffResul[$row->kode_kota] = $row->kota;
            }
        }
        return $xBuffResul;
    }
