function dosearchthread(xAwal) {
    ...
    $(document).ready(function () {
        $.ajax({
            url: getBaseURL() + "index.php/ctrthread/searchthread/",
            data: "xAwal=" + xAwal + "&xSearch=" + xSearch,
            cache: false,
            dataType: 'json',
            type: 'POST',
            success: function (json) {
                $("#tabledatathread").html(json.tabledatathread);
...
function doeditthread(edidx) {
    $(document).ready(function () {
        $.ajax({
            url: getBaseURL() + "index.php/ctrthread/editrecthread/",
            data: "edidx=" + edidx,
            cache: false,
            dataType: 'json',
            type: 'POST',
            success: function (json) {
                var val = json.event.split(',');
                $("#edidx").val(json.idx);
                $("#edtitleindo").val(json.title_ind);
                $("#edtitleeng").val(json.title_eng);
                $("#eddescriptionindo").val(json.content_ind);
                $("#eddescriptioneng").val(json.content_eng);
                $('#edidevent').select2().val(val).trigger('change');
                $('.select2').css("width", "");
...
function doClearthread() {
    $(document).ready(function () {
        ...
        $("#edidkategori").val("");
        $('#edidevent').select2().val("").trigger('change');
        $("#edtanggal").val("");
    });
}
...
function dosimpanthread() {
    $(document).ready(function () {
        $.ajax({
            url: getBaseURL() + "index.php/ctrthread/simpanthread/",
            data: 
                "edidx=" + $("#edidx").val() + 
